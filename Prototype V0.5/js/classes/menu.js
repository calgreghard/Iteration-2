    ...
    update(context, city) {
        ...
        if (this.visible) {
            ...
            switch (this.tabs[this.tabIndex].style) {
                case menuType.TRADE:
                    ...
                    if (!city.playerCaravans[city.selectedVessel].active) {
                        for (var i = 0; i < city.properties.length; i++) {
                            ...
                            this.selections[1][i].content = `${city.properties[i].produce.name[0].toUpperCase()}${city.properties[i].produce.name.substr(1)}`;

                            if (city.selectedProduct[city.selectedVessel] === i) {
                                this.selections[1][i].bgColour = 'rgb(255,255,240)';
                                this.chosenProduct = Object.assign({}, city.properties[i].produce);
                                this.productBaseValue = this.chosenProduct.value;
                            } else {
                        ...
                        var j = 0;
                        for (var i = 0; i < this.selections[2].length; i++) {
                    ...
                    }/*Loop through each selection*/ else {
                    ...
                    if (this.chosenProduct[0] === goodType.SCRIPTURE) {
                        this.chosenProduct.value /= .68;
                    }
                    if (!city.playerCaravans[city.selectedVessel].active) {
                        if (city.patronDeity && this.chosenProduct && this.chosenCity.religion === city.religion) {
                            for (var i = 0; i < this.chosenProduct.type.length; i++) {
                                let x = false;
                                for (var l = 0; l < city.patronDeity.length; l++) {
                                    for (var j = 0; j < city.patronDeity[l].type.length; j++) {
                                        if (this.chosenProduct.type[i] === city.patronDeity[l].type[j]) {
                                            this.chosenProduct.value /= .68;
                                            x = true;
                                            break;
                                        }
                        ...
                        this.tradeInfo.content = Math.round((this.chosenProduct.value * this.chosenCity.multiplier) - (this.productBaseValue * city.multiplier)) + ' gold in ' + Math.round(city.routeLength(this.chosenCity)) + ' miles';
                ...
                case menuType.INFO:
                    var text = new label(new point(this.tabPoints[0].x, this.location.y + (2 * this.size.y / 10)),
                        city.description, 'rgb(0,0,0)');
                    text.draw(context);
                    break;
            }
        }
